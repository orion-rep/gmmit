...
func generatePrompt(commitStandard, gitBranch, gitDiff, typeHint string) string {
	typeConstraint := ""
	if typeHint != "" {
	...
	If there are many changes, list the rest at the end.%s

	These are the file changes to be pushed:\n%s`, systemPrompt, commitStandard, gitBranch, typeConstraint, gitDiff)
}
...
func GenerateCommitMessage() {
	...
	prompt = generatePrompt(commitStandard, gitBranch, gitDiff, commitTypeHint)
